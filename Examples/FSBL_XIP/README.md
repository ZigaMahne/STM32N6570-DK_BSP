...
# FSBL_XIP Example for STM32N6570-DK_BSP board

The **FSBL_XIP** project is a simple example that can be used to verify the
basic tool setup.

Project provides a reference First Stage Bootloader with Execute in Place (XIP) example that can be used to build any firmware application to execute in external Flash (Appli).
...
The bootROM copies FSBL image from external Flash (Octo SPI Flash Memory) into internal RAM (AXI SRAM2) and begins execution to initialize the caches and configure the clocks. Then configures the external Flash (Octo SPI Flash Memory) in execution mode and the application begins execution.
...
- STM32N6570-DK Board is configured in [STM32CubeMX](https://github.com/Open-CMSIS-Pack/STM32N6570-DK_BSP/blob/main/Examples/FSBL_XIP/Configuration.md#in-stm32cubemx)
- STM32N6570-DK Board is configured in [VSCode](https://github.com/Open-CMSIS-Pack/STM32N6570-DK_BSP/blob/main/Examples/FSBL_XIP/Configuration.md#configuration-in-vscode)
...
- Ensure **ST-Link@pyOCD** Debug Adapter is selected and **Update launch.json and tasks.json** checkbox is selected and click **Save** then click **Build solution**
    ...
    - FSBL_Appli build also generates `FSBL.hex` and `FSBL.axf` in the project root folder
    ...
    - **FLASH MODE**
        ...
        - Open `.vscode\launch.json` file and modify configuration named "STLink@pyOCD (launch)"
            ...
            "initCommands": [
                ...
                "continue",
                "symbol-file \"out/Appli/STM32N657X0HxQ/Debug/Appli.axf\"",
                "tbreak main"
            ```
